...
LETTER_SCORE = {
  ...
  def self.build_score(words)
    ...
    sorted_score_hash.find_all { |letter, score| score == highest_score}.sort_by { |letter,score| letter.length }
  end

  def self.sort_score_hash(score_hash)
    Hash[score_hash.sort_by { |letter, score| score }]
  end
end
